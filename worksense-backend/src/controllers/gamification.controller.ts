 ...
 */
export const getUserGamificationData: RequestHandler = async (req, res, next) => {
    try {
...
export const getProjectGamificationStats: RequestHandler = async (req, res, next) => {
    try {
        const { projectId } = req.params;
        ...
        const stats = {
            ...
            totalPoints: users.reduce((sum: number, user: any) => sum + (user.points || 0), 0),
            averagePoints: users.length > 0 ? Math.round(users.reduce((sum: number, user: any) => sum + (user.points || 0), 0) / users.length) : 0,
            topPerformer: users.length > 0 ? users.reduce((top: any, user: any) => (user.points || 0) > (top.points || 0) ? user : top, users[0]) : null,

            // Project stats
            totalBacklogItems: backlogItems.length,
            completedTasks: backlogItems.filter((item: any) => item.status === 'done').length,
            inProgressTasks: backlogItems.filter((item: any) => item.status === 'in-progress').length,
            todoTasks: backlogItems.filter((item: any) => item.status === 'todo' || !item.status).length,

            // Calculate completion rate
            get completionRate() {
                return this.totalBacklogItems > 0 ? Math.round((this.completedTasks / this.totalBacklogItems) * 100) : 0;
            },
            ...
            techTasks: backlogItems.filter((item: any) => item.type === 'techTask').length
        };
