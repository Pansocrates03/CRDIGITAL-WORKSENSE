...
const BacklogTablePage: FC = () => {
      ...
      try {
        ...
        if (epicStories.length > 0) {
          const deleteStoryPromises = epicStories.map((story) =>
            apiClient.delete(
              `/api/v1/${projectId}/backlog/items/${story.id}?type=story`
            )
          );
        ...
        await apiClient.delete(
          `/api/v1/${projectId}/backlog/items/${epicId}?type=epic`
        );
  ...
  const handleDelete = async (item: BacklogItem) => {
    if (
      ...
      try {
        await apiClient.delete(
          `/api/v1/${projectId}/backlog/items/${item.id}?type=${item.type}`
        );
