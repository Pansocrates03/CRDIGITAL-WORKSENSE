...
const BacklogTablePage: FC = () => {
  ...
  const {
    ...
    return {
      ...
      standaloneStories: itemArray.filter((i) => i.type === "story" && !i.epicId),
  ...
  const handleDeleteEpic = async (epicId: string) => {
    ...
    const message = epicStories.length > 0 
      ...
      try {
        ...
        if (epicStories.length > 0) {
          const deleteStoryPromises = epicStories.map(story => 
            apiClient.delete(`/api/v1/${projectId}/backlog/items/${story.id}?type=story`)
          );
          await Promise.all(deleteStoryPromises);
        }
        
        // Después eliminamos la épica
        await apiClient.delete(`/api/v1/${projectId}/backlog/items/${epicId}?type=epic`);
        
        // Refrescamos los datos
        refetch();
      } catch (error) {
        console.error('Error al eliminar la épica:', error);
        alert('Ocurrió un error al eliminar la épica. Por favor, inténtalo de nuevo.');
      }
    }
  };
  ...
  useEffect(() => {
    ...
    const epicsToExpand = epics.filter(epic => {
  ...
  const shouldShowStoryInEpic = (story: BacklogItem, epic: BacklogItem) => {
    ...
    if (story.title.toLowerCase().includes(searchTerm.toLowerCase())) return true;
    
    // Si la épica coincide, mostrar todas sus historias
    if (epic.title.toLowerCase().includes(searchTerm.toLowerCase())) return true;
  ...
  const handleDelete = async (item: BacklogItem) => {
    if (window.confirm(`¿Estás seguro de que deseas eliminar "${item.title}"?`)) {
      try {
        await apiClient.delete(`/api/v1/${projectId}/backlog/items/${item.id}?type=${item.type}`);
  ...
  const renderRow = (item: BacklogItem, indent = false) => {
    let extraInfo = "-";
    if (item.type === "story") {
      extraInfo = item.storyPoints !== undefined && item.storyPoints !== null
      ...
      <div className={styles.tableContainer}>
        <table className={styles.table}>
          ...
          <tbody>
            ...
            {epics.length === 0 ? (
              <tr><td colSpan={5}>No epics found</td></tr>
            ) : (
              epics.map(epic => {
                const epicStories = stories.filter(s => s.epicId === epic.id);
                ...
                return (
                  <React.Fragment key={epic.id}>
                    ...
                    />
                    {expandedEpics.includes(epic.id) && epicStories
            ...
            {standaloneStories.filter(matchesSearch).length === 0 ? (
              <tr><td colSpan={5}>No standalone stories found</td></tr>
            ) : (
              standaloneStories.filter(matchesSearch).map(story => renderRow(story))
            )}
            ...
            {techTasks.filter(matchesSearch).length === 0 ? (
              <tr><td colSpan={5}>No tech tasks found</td></tr>
            ) : (
            ...
            {knowledge.filter(matchesSearch).length === 0 ? (
              <tr><td colSpan={5}>No knowledge items found</td></tr>
            ) : (
