...
import { useSprints } from "@/hooks/useSprintData"; // Keep this import from 'main'
...
const BacklogTablePage: FC = () => {
  ...
  // States from 'main' branch for success/error
  ...
  const hasPermissions =
    projectRole === "product-owner" || projectRole === "scrum-master";
  ...
  const handleDelete = (item: BacklogItemType) => {
    ...
    setDeleteModalMessage(
      `Deleting ${item.name} is a permanent action and cannot be undone.`
    );
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      handleSuccess(
        `${itemToDelete.name} successfully deleted`,
        "You should no longer see this item in the backlog."
      );
      refetch();
    } catch (error) {
  ...
  const handleStoriesAdded = (epicId: string) => {
    ...
    handleSuccess(
      ...
      "You should now see the stories in the backlog under the epic."
    );
        ...
        <ItemDetailsModal
          ...
          linkedEpic={
            selectedItem?.parentId
              ? categorized.epics.find(
                  (epic) => epic.id === selectedItem.parentId
                ) || null
              : null
          }
