...
import {
  FiLayout,
  FiGrid,
  FiBarChart, // Icons for tab navigation
} from "react-icons/fi";
...
const WorkflowPage: React.FC = () => {
  ...
  React.useEffect(() => {
    if (
      project &&
      Array.isArray(project.workflowStages) &&
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            There are no active sprints in this project. Please create or
            activate a sprint to start managing your workflow.
          </p>
        </div>
      </div>
    );
  ...
  const getItemChildren = (item: BacklogItemType): BacklogItemType[] => {
    ...
    return [item, ...item.subItems.flatMap(getItemChildren)];
  };
  ...
  React.useEffect(() => {
    if (data && activeSprint) {
      ...
      let filteredData = flattenedData.filter(
        (item) => item.type !== "epic" && item.sprint === activeSprint.id
      );
  ...
  const handleTaskUpdate = async (
    ...
    // Use the mutation hook for both regular items and subitems
    updateBacklogItemMutation.mutate({
      projectId: projectId ? projectId : "",
      itemId: taskId,
      itemType: task.type || "story", // fallback to "story" if no type
      updateData: { status: newStatus },
      parentId: task.parentId, // pass parentId for subitems
    });
  };
  ...
  const onTaskContentUpdate = async (
    ...
    setTasks((prevItems) =>
      prevItems.map((item) =>
        item.id === backlogItemId ? { ...item, tasks: newTasks } : item
      )
    );
  ...
  const burndownChartData = React.useMemo(() => {
    ...
    return createBurndownChartData(
      tasks,
      sprintStart,
      sprintEnd,
      storyPointScale
    ...
    allItems.forEach((item) => {
      ...
      if (typeof updatedAt === "object" && updatedAt !== null) {
        if (
          ...
          typeof (updatedAt as any).seconds === "number"
        ) {
          updatedAt = new Date((updatedAt as any).seconds * 1000).toISOString();
        } else if (
          "_seconds" in updatedAt &&
          typeof (updatedAt as any)._seconds === "number"
        ) {
          updatedAt = new Date(
            (updatedAt as any)._seconds * 1000
          ).toISOString();
        }
      }
  ...
  if (error) {
    throw new Error("An error has occurred on SprintPage.tsx");
  }
  if (isLoading) {
      ...
      case "board":
        return (
          <BoardView
            ...
            columns={columns}
            onTaskContentUpdate={onTaskContentUpdate}
          />
        );
      case "overview":
    ...
    >
      <div className="sprint-page__header">
        <div className="sprint-page__header-content">
          ...
          <p className="sprint-page__description">
            Sprint board for tracking project tasks and progress
          </p>
        </div>
      </div>
      ...
      {projectId && (
        <CreateItemModal
          ...
          statusOptions={
            project?.workflowStages || ["To Do", "In Progress", "Done"]
          }
        />
      )}
      ...
      {projectId && (
        <UpdateItemModal
          ...
          onItemUpdated={() => {
            handleSuccess(
              `The ${itemToEdit?.type} ${itemToEdit?.name} was updated successfully!`,
              "You should now see the updated item in the backlog."
            );
            refetch();
          }}
          onError={handleError}
