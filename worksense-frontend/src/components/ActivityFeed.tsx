...
const ActivityFeed: React.FC<ActivityFeedProps> = ({
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      case "badge_earned":
        return (
          <Award className="w-4 h-4" style={{ color: "var(--accent-pink)" }} />
        );
      default:
        return (
  ...
  if (!activities || activities.length === 0) {
    return (
      <div className="text-center py-8" style={{ color: "var(--neutral-600)" }}>
  ...
  return (
    <div className="space-y-3">
      {activities.map((activity: any, index: number) => (
        <div
          key={index}
          className="flex gap-3 p-3 rounded-lg transition-colors bg-neutral-50"
        >
