...
export const ProjectView: React.FC<FullProjectData> = ({
  ...
  const { data: leaderboard, isLoading: leaderboardLoading } = useProjectLeaderboard(projectId!, 3);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch backlog items
        const backlogResponse = await apiClient.get(`/projects/${projectId}/backlog/items`);
...
// Debug logging (remove in production)
  ...
  console.log("ProjectStats totalBacklogItems:", projectStats?.totalBacklogItems);
  console.log("Status breakdown:", {
    ...
    other: allWorkItems.filter(item => !isDone(item.status) && !isInProgress(item.status)).length
  });

// ALWAYS use the corrected calculation, not the API stats (which counts epics incorrectly)
  const completedTasks = allWorkItems.filter((item: any) => isDone(item.status)).length;
  const totalTasks = allWorkItems.length; // Use calculated total, not API total
  const inProgressTasks = allWorkItems.filter((item: any) => isInProgress(item.status)).length;
            ...
            <div className={styles.projectDetails}>
              <div className={styles.titleRow}>
                <button
                    ...
                    aria-label={isAboutExpanded ? "Hide project description" : "Show project description"}
                >
                  <ChevronDown
                ...
                <div className={"space-inline-x-2 flex items-center justify-between w-full"}>
              ...
              <span>{getOwnerName(project, members)} • {members.length} members</span>
        ...
        <div className={styles.dashboard}>
            ...
            <div className={styles.progressStats}>
              <div className={styles.statRow}>
                <div className={styles.stat}>
                  <Zap className="w-4 h-4" style={{ color: 'var(--accent-pink)' }} />
                ...
                <div className={styles.stat}>
                  <Target className="w-4 h-4" style={{ color: 'var(--accent-blue)' }} />
                  ...
                  <div className="space-y-2">
                    {leaderboard.map((contributor: any, index: number) => (
                      ...
                      <span
                          ...
                          style={{ color: index === 0 ? 'var(--accent-pink)' : 'var(--text-primary)' }}
                      >
          ...
          <div className={styles.activityPanel}>
            ...
            {projectId ? (
                ...
                <div style={{ color: 'var(--neutral-600)' }}>Loading activity...</div>
            )}
                ...
                {loading ? (
                    ...
                    backlogItems.slice(0, 3).map((item) => (
                        <div key={item.id} className={styles.taskItem}>
                          <span className={`${styles.taskStatus} ${styles[item.status]}`}></span>
